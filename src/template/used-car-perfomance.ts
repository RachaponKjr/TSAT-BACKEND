...
<body>
  ...
  <div style="border: 1px solid #ccc; border-radius: 6px; padding: 20px; font-family: sans-serif; color: #333; margin-bottom: 8px; font-size: 12px; line-height: 1.4;">
    ...
    <div style="display: flex; margin-bottom: 16px;">
      ...
      <div style="flex: 0 0 35%; padding-right: 10px;">
        <div style="font-size: 0.85em; color: #777; margin-bottom: 4px;">ลูกค้า</div>
        <div style="font-weight: bold; font-size: 1.15em; color: #222;">คุณ${
          data.customerName
        }</div>
      </div>
      <!-- คอลัมน์ 2: รุ่นรถ -->
      <div style="flex: 0 0 40%; padding-right: 10px;">
        <div style="font-size: 0.85em; color: #777; margin-bottom: 4px;">รุ่นรถ</div>
        <div style="font-weight: bold; font-size: 1.15em; color: #222;">Porsche ${
          data.carModel
        }</div>
      </div>
      <!-- คอลัมน์ 3: รหัสตัวถัง -->
      <div style="flex: 0 0 25%;">
        <div style="font-size: 0.85em; color: #777; margin-bottom: 4px;">รหัสตัวถัง (VIN)</div>
        <div style="font-weight: bold; font-size: 1.15em; color: #222;">${
          data.vin
        }</div>
      </div>
    </div>
    ...
    <div style="display: flex; align-items: flex-start;">
      ...
      <div style="flex: 0 0 35%; display: flex; gap: 24px; padding-right: 10px;">
        <div>
          <div style="font-size: 0.85em; color: #777; margin-bottom: 4px;">ทะเบียน</div>
          <div style="font-weight: bold; font-size: 1.15em; color: #222;">${
            data.licensePlate
          }</div>
        </div>
        <div>
          <div style="font-size: 0.85em; color: #777; margin-bottom: 4px;">ปีรถ</div>
          <div style="font-weight: bold; font-size: 1.15em; color: #222;">${
            data.modelYear
          }</div>
        </div>
        <div>
          <div style="font-size: 0.85em; color: #777; margin-bottom: 4px;">เลขกิโลเมตร</div>
          <div style="font-weight: bold; font-size: 1.15em; color: #222;">${
            data.odometer
          } กม.</div>
        </div>
      ...
      <div style="flex: 0 0 40%; padding-right: 10px;">
        <div style="font-size: 0.85em; color: #777; margin-bottom: 4px;">ผู้ตรวจเช็ค</div>
        <div style="font-weight: bold; font-size: 1.15em; color: #222;">
          ${
            data.inspectorName
          } <span style="font-weight: normal; color: #444; margin-left: 12px; font-size: 0.9em;">${formatThaiDate(
            data.inspectedAt
          )}</span>
        </div>
      </div>
  ...
  <div style="width:100%;display:flex;gap:24px">
      <!--ภาพรูปรถ-->
      <div style="flex-grow:1; background-color: lightblue;border-radius: 6px;overflow:hidden">
      ...
      <div style="display:flex;flex-direction: column; gap:32px;flex-basis:18rem;flex-shrink:0">
          ...
          <div style="display:flex;flex-direction: column;color:#666666;">
              <span style="font-size:12px">ผลการประเมิน</span>
              <span style="font-size:16px;font-weight: bold">${getGradeLabelUsedCar(
                data?.overallGrade
              )}</span>
          </div>
  ...
  <div style="display:flex; flex-direction: column;">
    <span style="font-size:12px; color:#666666; margin-bottom: 4px;">Overall Grade</span>
    <span style="color:${getGradeColor(
      data?.overallGrade
    )}; font-size:36px; font-weight:bold; line-height:0.9;">${data?.overallGrade}</span>
    <span style="color:${getGradeColor(
      data?.overallGrade
    )}; font-size:13px; font-weight:bold; margin-top: 2px;">${getGradeLabel(
      data?.overallGrade
    )}</span>
  </div>
  ...
  <!-- กล่องคะแนนรวม -->
  <div style="display:flex; flex-direction: column;">
    <span style="font-size:12px; color:#666666; margin-bottom: 4px;">คะแนนรวม</span>
    <span style="color:${getGradeColor(
      data?.overallGrade
    )}; font-size:36px; font-weight:bold; line-height:0.9;">${data?.totalScore}</span>
    <span style="color:#999999; font-size:13px; font-weight:bold; margin-top: 2px;">/ ${data?.maxScore}</span>
  </div>

</div>
          </div>
          <div style="display:flex;flex-direction: column;font-size:10px;color:#999999;gap:6px;">
